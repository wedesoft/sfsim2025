...
state_t *state_change(state_t *s, body_t b, forces_t f, vector_t linear_impulse, vector_t angular_impulse, double dt) {
  ...
  vector_t rotation_change_ = rotation_change(s, b, f.torque, angular_impulse, dt);
...
void *world_change(double time, double dt, void *world_, void *data_) {
  ...
  for (int c=0; c<data->iterations; c++) {
    ...
    for (int k=0; k<data->joints.size; k++) {
      ...
      state_t *prediction1 = predict(s1, body1, f1.force, f1.torque, linear_impulse[i], angular_impulse[i], dt);
      state_t *prediction2 = predict(s2, body2, f2.force, f2.torque, linear_impulse[j], angular_impulse[j], dt);
    ...
    for (int k=0; k<contacts_.size; k++) {
      ...
      if (is_resting(c, data->resting_threshold) == (dt != 0)) {
        ...
        state_t *contact_prediction1 = predict(s1, body1, f1.force, f1.torque, linear_impulse[i], angular_impulse[i], dt);
        state_t *contact_prediction2 = predict(s2, body2, f2.force, f2.torque, linear_impulse[j], angular_impulse[j], dt);
        ...
        state_t *friction_prediction1 = predict(s1, body1, f1.force, f1.torque, linear_impulse[i], angular_impulse[i], dt);
        state_t *friction_prediction2 = predict(s2, body2, f2.force, f2.torque, linear_impulse[j], angular_impulse[j], dt);
