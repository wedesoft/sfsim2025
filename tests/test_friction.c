...
static MunitResult test_pass_anisotropic(const MunitParameter params[], void *data) {
  friction_t f = anisotropic_friction(5, 7);
  large_vector_t v = allocate_large_vector(2);
  v.data[0] = 3; v.data[1] = 6;
  large_vector_t l = limit_friction(f, 1, v);
  munit_assert_int(l.rows, ==, 2);
  munit_assert_double(l.data[0], ==, 3);
  munit_assert_double(l.data[1], ==, 6);
  return MUNIT_OK;
}

static MunitResult test_limit_anisotropic(const MunitParameter params[], void *data) {
  friction_t f = anisotropic_friction(2, 3);
  large_vector_t v = allocate_large_vector(2);
  v.data[0] = 5; v.data[1] = 7;
  large_vector_t l = limit_friction(f, 2, v);
  munit_assert_int(l.rows, ==, 2);
  munit_assert_double(l.data[0], ==, 4);
  munit_assert_double(l.data[1], ==, 6);
  return MUNIT_OK;
}
...
MunitTest test_friction[] = {
  ...
  {"/pass_anisotropic"    , test_pass_anisotropic    , NULL, NULL, MUNIT_TEST_OPTION_NONE, NULL},
  {"/limit_anisotropic"   , test_limit_anisotropic   , NULL, NULL, MUNIT_TEST_OPTION_NONE, NULL},
