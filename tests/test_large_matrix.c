...
{
  munit_assert_int(projection(320, 240, 1, 100, 90).rows, ==, 4);
  munit_assert_int(projection(320, 240, 1, 100, 90).cols, ==, 4);
  munit_assert_double_equal(projection(320, 240, 1, 100, 90).data[0], 1, 6);
  munit_assert_double_equal(projection(320, 240, 1, 100, 60).data[0], sqrt(3.0f), 6);
  return MUNIT_OK;
}

static MunitResult test_vertical_fov(const MunitParameter params[], void *data)
{
  munit_assert_double_equal(projection(320, 320, 1, 100, 60).data[5], sqrt(3.0f), 6);
  munit_assert_double_equal(projection(320, 240, 1, 100, 60).data[5], 4.0f / sqrt(3.0f), 6);
  return MUNIT_OK;
}

static MunitResult test_perspective(const MunitParameter params[], void *data)
{
  munit_assert_float(projection(320, 240, 0.1f, 100, 60).data[14], ==, -1.0f);
  munit_assert_float(projection(320, 240, 1, 3, 60).data[10], ==, -2.0f);
  munit_assert_float(projection(320, 240, 1, 5, 60).data[10], ==, -1.5f);
  munit_assert_float(projection(320, 240, 1, 3, 60).data[11], ==, -3.0f);
  munit_assert_float(projection(320, 240, 1, 5, 60).data[11], ==, -2.5f);
  return MUNIT_OK;
}
